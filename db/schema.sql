...
CREATE TABLE `instructors` (
  ...
  `avatar` VARCHAR(512) NULL COMMENT '头像文字或 URL',
...
CREATE TABLE `redeem_codes` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `code` VARCHAR(32) NULL COMMENT '旧版明文兑换码（兼容存量数据，新数据优先使用 code_hash）',
  `code_hash` CHAR(64) NULL COMMENT '兑换码 SHA-256 哈希',
  `code_suffix` VARCHAR(8) NULL COMMENT '兑换码后缀，便于客服核对',
  `entitlement_id` BIGINT NULL COMMENT '绑定的课程权益ID',
  `course_id` BIGINT NOT NULL COMMENT '关联的课程ID',
...
-- ------------------------------------------------------------
-- 表名：wechat_store_orders（微信小店订单承接表）
-- ------------------------------------------------------------
DROP TABLE IF EXISTS `wechat_store_orders`;
CREATE TABLE `wechat_store_orders` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
-- ------------------------------------------------------------
-- 表名：wechat_store_order_items（微信小店订单明细表）
-- ------------------------------------------------------------
DROP TABLE IF EXISTS `wechat_store_order_items`;
CREATE TABLE `wechat_store_order_items` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `store_order_id` VARCHAR(64) NOT NULL COMMENT '微信小店订单号',
...
-- ------------------------------------------------------------
-- 表名：claim_tokens（微信小店购后领取 token 表）
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 表名：fulfillment_logs（微信小店购后发货日志表）
-- ------------------------------------------------------------
DROP TABLE IF EXISTS `fulfillment_logs`;
