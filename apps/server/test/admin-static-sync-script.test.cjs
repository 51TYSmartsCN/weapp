...
const adminPackageJsonPath = path.resolve(__dirname, '../../admin/package.json')
...
test('root build:admin script syncs admin dist into server public admin', () => {
  ...
  assert.equal(rootPackageJson.scripts['build:admin'], 'pnpm --filter @geo/admin build')
  assert.equal(
    adminPackageJson.scripts.build,
    'tsc -b && vite build && bash ../../scripts/sync-admin-static.sh',
  )
})
