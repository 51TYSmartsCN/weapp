...
test('runner deploy script keeps build, deploy, and admin page verification in one flow', () => {
  ...
  assert.match(deployScript, /HTTP_WAIT_ATTEMPTS="\$\{HTTP_WAIT_ATTEMPTS:-10\}"/)
  assert.match(deployScript, /HTTP_WAIT_SECONDS="\$\{HTTP_WAIT_SECONDS:-3\}"/)
  assert.match(deployScript, /wait_for_http[\s\\\n"]+"remote loopback health"/)
  assert.match(deployScript, /wait_for_http[\s\\\n"]+"public health"/)
  assert.match(deployScript, /bash -lc "\$command"/)
